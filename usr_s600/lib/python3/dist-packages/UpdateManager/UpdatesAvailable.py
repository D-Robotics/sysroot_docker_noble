...
class UpdatesAvailable(InternalDialog):
    ...
    def __init__(self, window_main, header=None, desc=None, need_reboot=False):
        ...
        self.button_pro = self.add_button(
            _("Enable Ubuntu Pro..."), self.on_button_pro_clicked
        )
        ...
        pkg_toggle_renderer.connect(
            "toggled", self.on_update_toggled, self.treeview_update
        )
    ...
    def on_treeview_update_cursor_changed(self, widget):
        ...
        # Screen reader: Speak state of toggle box
        ac = widget.get_accessible()
        if model.get_value(iter, LIST_TOGGLE_ACTIVE):
            ac.emit("notification", _("Selected"), 0)
        else:
            ac.emit("notification", _("Unselected"), 0)
        ...
        try:
            ...
            if self.dl_size != 0:
                ...
                if inst_count > 0:
                    ...
                    download_str = _("You need to enable Ubuntu Pro to install"
                                     " these updates.")
    ...
    def on_update_toggled(self, renderer, path, treeview):
        ...
        # Screen reader: Speak state of toggle box
        ac = treeview.get_accessible()
        if self.store.get_value(iter, LIST_TOGGLE_ACTIVE):
            ac.emit("notification", _("Selected"), 0)
        else:
            ac.emit("notification", _("Unselected"), 0)
    ...
    def on_treeview_update_row_activated(self, treeview, path, column, *args):
        ...
        install check box
        """
        self.on_update_toggled(None, path, treeview)
    ...
    def _add_groups(self, groups):
        ...
        for group in groups:
            ...
            if len(group.items) > 1:
                name = '%s (%d)' % (group.name, len(group.items))
