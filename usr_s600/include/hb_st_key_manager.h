...
typedef struct {
	...
	*/
	...
	5 从RPMB读出后，注入TEE新生密钥Encrypt_AES_Key：key_st_type==0x1=0b0001，需要tee使用Ultra KEK解密
	6 从RPMB读出后，注入TEE新生密钥Reset_AES_Key：key_st_type==0x1=0b0001,需要tee使用Ultra KEK解密
	*/
 ...
 *
 * @compatibility HW: Ultra/SuperSoC
 *
 ...
 *
 * @param[in] key_list: key info list 输出key信息的位置
 * @param[in] key_list_size: key_list size 输出key_list可用大小以及实际使用大小
 *
 * @compatibility HW: SuperSoC
 *
 * @compatibility SW: 1.1.0
