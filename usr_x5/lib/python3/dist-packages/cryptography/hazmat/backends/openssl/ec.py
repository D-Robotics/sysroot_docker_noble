...
def _check_key_cofactor(backend: Backend, ec_cdata) -> None:
    group = backend._lib.EC_KEY_get0_group(ec_cdata)
    backend.openssl_assert(group != backend._ffi.NULL)
    bn = backend._lib.EC_GROUP_get0_cofactor(group)
    cofactor = backend._bn_to_int(bn)
    if cofactor != 1:
        if backend._lib.EC_KEY_check_key(ec_cdata) != 1:
            raise ValueError(
...
class _EllipticCurvePublicKey(ec.EllipticCurvePublicKey):
    def __init__(self, backend: Backend, ec_key_cdata, evp_pkey):
        ...
        _mark_asn1_named_ec_curve(backend, ec_key_cdata)
        _check_key_infinity(backend, ec_key_cdata)
        _check_key_cofactor(backend, ec_key_cdata)
